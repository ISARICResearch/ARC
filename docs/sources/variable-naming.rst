...
This document describes the variable naming conventions used in ARC, which support consistent structure, clarity, and integration with data collection and harmonization tools like `ISARIC BRIDGE <https://bridge.isaric.org>`_ and `ISARIC VERTEX <https://vertex.isaric.org>`_.
...
Such fields allow flexibility across settings while preserving data harmonization. The recommended unit (standardized) is often **pre-selected** via presets in `BRIDGE <https://github.com/ISARICResearch/ARC>`_.